...
function makeCards(numOfPlayers) {
	...
	for (let i = 0; i < numOfPlayers; i++) {
		cards[`Player${i+1}Cards`] = cards.deck.splice(0, cardsPerPlayer);
	}

	//define a method for a player to draw from closed deck
	cards.closedDraw = function (playerNo) {
		cards[`Player${playerNo}Cards`].push(cards.deck.splice(0, 1));
	}

	//define a method for a player to draw from open deck
	cards.openDraw = function (playerNo) {
		cards[`Player${playerNo}Cards`].push(cards.openDeck.splice(0, 1));
	}

	//define a method for a player to deposit one of their cards onto the open deck
	cards.depositCard = function (playerNo, cardNo) {
		cards['openDeck'].push(cards[`Player${playerNo}Cards`].splice(cardNo, cardNo + 1)[0]);
	}
